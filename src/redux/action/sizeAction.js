...
export const createSize = (params) => async (dispatch,getState) => {
  try {
    ...
    const response = await axios.post('http://localhost:3300/api/v1/sizes/create-size', params, config);
...
export const listeSizes = () => async (dispatch) => {
    try {
      ...
      const response = await axios.get('http://localhost:3300/api/v1/sizes/all');
